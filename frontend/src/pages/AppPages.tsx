...
import { Book, BookStatus, ReadingAnalytics, ReadingInsight, ReminderSettings, WishlistItem } from '../types'
...
export function Dashboard() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      api.get<{ items: ReadingInsight[] }>('/dashboard/insights').then((r) => setInsights(r.data.items)),
    ...
    <div className='space-y-6'>
      ...
      <div className='grid gap-4 lg:grid-cols-3'>
        <SectionCard className='lg:col-span-2'>
          ...
          {!analytics || loading ? <Skeleton className='h-40' /> : (
            <div className='space-y-4'>
              <div className='grid gap-3 sm:grid-cols-2 xl:grid-cols-4'>
                <div className='metric-tile'><p className='text-small text-mutedForeground'>{t('dashboard.totalPagesRead')}</p><p className='text-2xl font-semibold'>{analytics.totalPagesRead}</p></div>
                <div className='metric-tile'><p className='text-small text-mutedForeground'>{t('dashboard.completionRate')}</p><p className='text-2xl font-semibold'>{analytics.completionRate}%</p></div>
                <div className='metric-tile'><p className='text-small text-mutedForeground'>{t('dashboard.readingPace')}</p><p className='text-2xl font-semibold'>{analytics.readingPacePerMonth}</p></div>
                <div className='metric-tile'><p className='text-small text-mutedForeground'>{t('dashboard.currentStreak')}</p><p className='text-2xl font-semibold'>{analytics.currentStreakWeeks}</p></div>
              </div>
              <div className='space-y-2'>
                {analytics.monthlyActivity.map((point) => (
                  <div key={point.label} className='space-y-1'>
                    <div className='flex items-center justify-between text-xs text-mutedForeground'><span>{point.label}</span><span>{point.count}</span></div>
        ...
        <SectionCard>
          ...
          {loading ? <Skeleton className='h-40' /> : (
            <div className='space-y-3'>
              {insights.map((item, idx) => <div key={idx} className='rounded-md border border-border bg-surface p-3 text-sm'>{item.message}</div>)}
              {reminder ? <div className='rounded-md border border-border bg-card p-3 text-sm text-mutedForeground'>{reminder.enabled ? t('dashboard.reminderOn', { time: reminder.time }) : t('dashboard.reminderOff')}</div> : null}
            </div>
          )}
        </SectionCard>
      </div>
      ...
      <Card className='p-4 text-sm text-mutedForeground'>{locale === 'fa' ? 'تحلیل‌ها بر اساس فعالیت ثبت‌شده شما در لیبرو هستند.' : 'Insights are derived from your logged Libro activity.'}</Card>
    </div>
  )
}
  ...
  return (<div className='space-y-6'>
    ...
    <SectionCard>
      ...
      <form onSubmit={add} className='grid gap-3 md:grid-cols-5'>
        ...
        <Select name='status' defaultValue='inLibrary'>{statusOptions.map((s) => <option key={s} value={s}>{t(`status.${s}`)}</option>)}</Select>
    ...
    <DataToolbar>
      ...
      <Select value={status} onChange={(e) => setStatus(e.target.value)} className='md:w-56'><option value=''>{t('library.allStatuses')}</option>{statusOptions.map((s) => <option key={s} value={s}>{t(`status.${s}`)}</option>)}</Select>
    </DataToolbar>

    {loading ? <div className='space-y-3'><Skeleton className='h-24' /><Skeleton className='h-24' /></div> : books.length ? <div className='space-y-3'>{books.map((b) => (
      <Card key={b.id} className='surface-hover p-5'><div className='flex flex-wrap items-center justify-between gap-3'><div><p className='text-lg font-semibold'>{b.title}</p><p className='text-small text-mutedForeground'>{b.author} • {b.totalPages}</p></div><div className='flex items-center gap-2'><StatusBadge status={b.status} /><Link to={`/books/${b.id}`}><Button size='sm'>{t('common.details')}</Button></Link><Button size='sm' variant='secondary' disabled={deletingId === b.id} onClick={() => void remove(b.id)}>{deletingId === b.id ? t('library.deleting') : t('books.delete')}</Button></div></div></Card>
    ))}</div> : <EmptyState icon='📚' title={t('library.noBooksTitle')} description={t('library.noBooksDescription')} action={<Button size='sm' onClick={() => { setSearch(''); setStatus(''); void load() }}>{t('library.clearFilters')}</Button>} />}
  </div>)
}
...
function BookListByStatus({ status, title, description }: { status: BookStatus; title: string; description: string }) {
  ...
  const [loading, setLoading] = useState(true)
  const { t } = useI18n()
  const load = async () => { setLoading(true); setBooks(await fetchBooks({ status })); setLoading(false) }
  useEffect(() => { void load() }, [status])
  return <div className='space-y-6'>
    ...
    {loading ? <div className='space-y-3'><Skeleton className='h-28' /><Skeleton className='h-28' /></div> : books.length ? books.map((b) => (
      <SectionCard key={b.id} className='surface-hover'>
        <div className='flex flex-wrap items-center justify-between gap-3'><div><p className='text-lg font-semibold'>{b.title}</p><p className='text-small text-mutedForeground'>{b.author}</p></div><p className='text-small text-mutedForeground'>{b.currentPage}/{b.totalPages}</p></div>
        <Progress value={b.progressPercentage} />
        {status === 'currentlyReading' ? <form className='grid gap-3 md:grid-cols-[1fr_auto_auto]' onSubmit={async (e) => { e.preventDefault(); const v = Number(new FormData(e.currentTarget).get('currentPage')); await api.patch(`/books/${b.id}/progress`, { currentPage: v }); void load() }}><Input type='number' name='currentPage' min={0} max={b.totalPages} placeholder={t('books.updateProgress')} /><Button type='submit'>{t('common.save')}</Button><Button variant='secondary' onClick={async () => { await api.patch(`/books/${b.id}/status`, { status: 'finished' }); void load() }}>{t('books.markFinished')}</Button></form> : null}
...
export function Wishlist() {
  ...
  useEffect(() => { void load() }, [])
  const add = async (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const f = new FormData(e.currentTarget); await api.post('/wishlist', { title: f.get('title'), author: f.get('author'), expectedPrice: f.get('expectedPrice') ? Number(f.get('expectedPrice')) : null, notes: f.get('notes') || null }); (e.target as HTMLFormElement).reset(); setError(''); void load() }
...
export function BookDetails({ id }: { id: string }) {
  ...
  return <div className='space-y-6'>
    ...
    <SectionCard>
      ...
      <div className='grid gap-3 text-small text-mutedForeground sm:grid-cols-3'><p>Total pages: {book.totalPages}</p><p>Current page: {book.currentPage}</p><p>Remaining: {book.remainingPages}</p><p>Progress: {Math.round(book.progressPercentage)}%</p><p className='sm:col-span-2'>{t('books.completed')}: {book.completedAt ? new Date(book.completedAt).toLocaleDateString() : t('books.notFinished')}</p></div>
      <Progress value={book.progressPercentage} />
    </SectionCard>
    <SectionCard>
      <h2 className='text-section-title'>{t('books.actions')}</h2>
      <div className='flex flex-wrap gap-2'>{statusOptions.map((s) => <Button key={s} variant='secondary' onClick={async () => { await api.patch(`/books/${book.id}/status`, { status: s }); void load() }}>{t('books.moveTo')} {t(`status.${s}`)}</Button>)}</div>
      ...
      <Button variant='secondary' disabled={deleting} onClick={async () => { if (!window.confirm(t('library.deleteConfirm'))) return; try { setDeleting(true); await deleteBook(book.id); nav('/library') } finally { setDeleting(false) } }}>{deleting ? t('library.deleting') : t('books.delete')}</Button>
    </SectionCard>
  </div>
}
...
export function Profile() {
  ...
  const [message, setMessage] = useState('')
  const [reminder, setReminder] = useState<ReminderSettings>({ enabled: false, time: '20:00', frequency: 'daily' })
  const { t } = useI18n()

  useEffect(() => { void api.get<ReminderSettings>('/users/reminders').then((r) => setReminder(r.data)) }, [])
  ...
  return (
    <div className='space-y-6'>
      ...
      <SectionCard className='max-w-2xl'>
        ...
        <form className='space-y-3' onSubmit={async (e) => { e.preventDefault(); await api.put('/users/profile', { name }); setMessage(t('profile.nameSuccess')) }}>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('profile.newName')} />
          <Button type='submit'>{t('profile.updateNameAction')}</Button>
        </form>
      </SectionCard>
      <SectionCard className='max-w-2xl'>
        <h2 className='text-section-title'>{t('profile.updatePassword')}</h2>
        <form className='space-y-3' onSubmit={async (e) => { e.preventDefault(); const f = new FormData(e.currentTarget); await api.put('/users/password', { currentPassword: f.get('currentPassword'), newPassword: f.get('newPassword') }); (e.target as HTMLFormElement).reset(); setMessage(t('profile.passwordSuccess')) }}>
          <Input type='password' name='currentPassword' placeholder={t('profile.currentPassword')} required />
          <Input type='password' name='newPassword' placeholder={t('profile.newPassword')} minLength={6} required />
          <Button type='submit'>{t('profile.updatePasswordAction')}</Button>
        </form>
      </SectionCard>
      <SectionCard className='max-w-2xl'>
        <h2 className='text-section-title'>{t('profile.reminders')}</h2>
        <form className='grid gap-3 md:grid-cols-3' onSubmit={async (e) => { e.preventDefault(); await api.put('/users/reminders', reminder); setMessage(t('profile.reminderSuccess')) }}>
          <label className='flex items-center gap-2 text-sm'><input type='checkbox' checked={reminder.enabled} onChange={(e) => setReminder((prev) => ({ ...prev, enabled: e.target.checked }))} /> {t('profile.reminderEnabled')}</label>
          <Input type='time' value={reminder.time} onChange={(e) => setReminder((prev) => ({ ...prev, time: e.target.value }))} />
          <Select value={reminder.frequency} onChange={(e) => setReminder((prev) => ({ ...prev, frequency: e.target.value as ReminderSettings['frequency'] }))}><option value='daily'>{t('profile.daily')}</option><option value='weekdays'>{t('profile.weekdays')}</option><option value='weekends'>{t('profile.weekends')}</option></Select>
          <Button type='submit' className='md:col-span-3 md:w-fit'>{t('profile.saveReminders')}</Button>
        </form>
      </SectionCard>
