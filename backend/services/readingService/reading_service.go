...
func (s *Service) GetGoalsOverview(ctx context.Context, userID uuid.UUID) (*GoalOverview, error) {
	...
	weeklyView := buildPeriodView("weekly", weeklyStart, weeklyEnd, weeklyGoal, events)
	monthlyView := buildPeriodView("monthly", monthlyStart, monthlyEnd, monthlyGoal, events)
	suggestions := s.buildSuggestions(ctx, userID, sessions, events)

	return &GoalOverview{Weekly: weeklyView, Monthly: monthlyView, Suggestions: suggestions}, nil
}

func buildPeriodView(period string, start, end time.Time, goal *readingGoal.ReadingGoal, events []readingEvent.ReadingEvent) GoalPeriodView {
	pagesRead, booksRead := aggregateInWindow(events, start, end)
	if pagesRead < 0 {
		pagesRead = 0
	}
	if booksRead < 0 {
...
func aggregateInWindow(events []readingEvent.ReadingEvent, start, end time.Time) (int, int) {
	pages := 0
	books := 0
...
func (s *Service) buildSuggestions(ctx context.Context, userID uuid.UUID, sessions []readingSession.ReadingSession, events []readingEvent.ReadingEvent) []GoalSuggestion {
	signals := collectSignals(sessions, events)
...
func collectSignals(sessions []readingSession.ReadingSession, events []readingEvent.ReadingEvent) Signals {
	now := time.Now()
